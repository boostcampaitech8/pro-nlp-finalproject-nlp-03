...
# [코드 리뷰] API 인증 전략
# 현재는 app 레벨에서 모든 엔드포인트에 API Key 인증을 적용하고 있습니다.
# 이는 /health 엔드포인트도 인증을 요구하게 됩니다.
#
# 질문: Health check 엔드포인트는 인증 없이 열어두는 것이 일반적인지,
# 아니면 보안을 위해 모든 엔드포인트에 인증을 적용하는 것이 맞는지 의견 부탁드립니다.
#
# 고려사항:
# - 옵션 A (현재): 모든 엔드포인트 인증 → 보안 강화, 일관성
#   하지만 모니터링/헬스체크 도구가 API Key 없이는 사용 불가
# - 옵션 B: /health만 인증 제외 → 모니터링 편의성, 업계 관례
#   하지만 서버 상태 정보가 노출될 수 있음 (정보 유출 위험)
# - 옵션 C: /health는 제한된 정보만 반환하고 인증 제외 → 균형점
#
# 실무에서는 어떤 방식을 선호하시는지 궁금합니다.
app = FastAPI(
    title="Qwen LLM API",
