...
[코드 리뷰] 베이스 클래스 설계
- 세 클라이언트 모두 requests를 직접 호출하고, 타임아웃/에러 처리/로그 포맷이 서비스마다 조금씩 다름.
- 실무에서는 BaseApiClient 같은 공통 베이스를 두고 _get/_post에 공통 에러 처리·로깅·재시도를 넣는
  패턴을 쓰는지, 아니면 지금처럼 클라이언트별 개별 처리로 두어도 괜찮은지 의견 부탁드립니다.
"""
...
# STT Client
# ============================================================================
# [코드 리뷰] health_check / transcribe에서 requests 직접 호출 + 예외 분기. TTS/LLM과 유사 패턴 반복.
...
# TTS Client
# ============================================================================
# [코드 리뷰] STT와 동일하게 requests 직접 호출. 에러 메시지/로그 문구만 서비스명 차이 (TTS vs STT).
...
# LLM Client
# ============================================================================
# [코드 리뷰] health_check + classify_and_respond 역시 동일 패턴. 공통 _get/_post 추출 시 중복 제거 가능.
