...
async def generate_recipe_from_chat(
    ...
    try:
        # 히스토리에서 원본 레시피 이미지 찾기
        original_image = None
        for msg in messages:
            if msg.get("role") == "assistant" and msg.get("image"):
                original_image = msg["image"]
                print(f"[Recipe API] 원본 이미지 발견: {original_image[:60]}...")
                break
        ...
        # 원본 이미지 우선 사용
        if original_image:
            print(f"[Recipe API] 원본 이미지 사용: {original_image[:60]}...")
            recipe_json['image'] = original_image
            recipe_json['img_url'] = original_image
